...
# Assuming perio represents the number of periods in a year (e.g., 12 for monthly data)
perio <- 12

# Realized Volatility for GMV using subsample2, annualized
realized_returns_gmv <- subsample2 %*% omega_gmv
realized_volatility_gmv_annualized <- sd(as.numeric(realized_returns_gmv)) * sqrt(perio)

# Sharpe Ratio for GMV, using annualized data
sharpe_ratio_gmv_annualized <- calculate_sharpe_ratio(as.numeric(realized_returns_gmv) * perio)

# Realized Volatility for PCA using subsample2, annualized
realized_returns_pca <- subsample2 %*% omega3
realized_volatility_pca_annualized <- sd(as.numeric(realized_returns_pca)) * sqrt(perio)

# Sharpe Ratio for PCA, using annualized data
sharpe_ratio_pca_annualized <- calculate_sharpe_ratio(as.numeric(realized_returns_pca) * perio)

# Update the data frame to store the annualized results
results_df <- data.frame(
  Strategy = c("GMV", "PCA"),
  Anticipated_Volatility = c(anticipated_volatility_gmv, anticipated_volatility_pca),  # This remains the same as it's already annualized
  Realized_Volatility_Annualized = c(realized_volatility_gmv_annualized, realized_volatility_pca_annualized),
  Sharpe_Ratio_Annualized = c(sharpe_ratio_gmv_annualized, sharpe_ratio_pca_annualized)
)
...
barplot(results_df$Anticipated_Volatility, names.arg = results_df$Strategy, main = "Anticipated Volatility")
barplot(results_df$Realized_Volatility_Annualized, names.arg = results_df$Strategy, main = "Annualized Realized Volatility")
barplot(results_df$Sharpe_Ratio_Annualized, names.arg = results_df$Strategy, main = "Annualized Sharpe Ratio")
...
# Calculate returns, risks, and Sharpe ratios
calculate_statistics <- function(simulated_values) {
  returns <- diff(simulated_values) / simulated_values[-length(simulated_values)]
  annual_return <- (simulated_values[length(simulated_values)] / simulated_values[1]) - 1
  annual_risk <- sd(returns) * sqrt(12)  # Assuming monthly returns, annualize the risk
  sharpe_ratio <- annual_return / annual_risk  # Assuming risk-free rate = 0
  
  return(c("Return" = annual_return, "Risk" = annual_risk, "Sharpe Ratio" = sharpe_ratio))
}

# Calculate statistics for each strategy
equity_stats <- calculate_statistics(mc_results$average_equity)
obpi_stats <- calculate_statistics(mc_results$average_obpi_call)
cppi_stats <- calculate_statistics(mc_results$average_cppi)

# Combine statistics into a data frame
monte_carlo_table <- data.frame(
  Strategy = c("Equity", "OBPI", "CPPI"),
  Return = c(equity_stats["Return"], obpi_stats["Return"], cppi_stats["Return"]),
  Risk = c(equity_stats["Risk"], obpi_stats["Risk"], cppi_stats["Risk"]),
  Sharpe_Ratio = c(equity_stats["Sharpe Ratio"], obpi_stats["Sharpe Ratio"], cppi_stats["Sharpe Ratio"])
)

# Print out the Monte Carlo simulation table
