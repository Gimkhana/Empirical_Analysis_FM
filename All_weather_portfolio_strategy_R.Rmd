...
7 (Appendix) represents the cumulative performance of the different
...
1920s, leading to colossal investor losses. Figure 8 (Appendix)
...
(Bridgewater Associates, LP, 2019). Figure 9 (Appendix) represents the
...
portfolio's components (Bridgewater Associates, LP, 2019). Figure 10
...
diversification (Bridgewater Associates, LP, 2009). Figure 11 (Appendix)
...
Associates, LP, 2009). Figure 12 (Appendix) represents the risk-return
...
even a negative correlation, is a driver of portfolio return. In this
sense, we can analyse in the below figure the effect of the correlation
on the risk incurred by the portfolio, where a correlation of 60% would
imply a probability of loss of funds of 38% annually, for a risk/return
ratio of 0.31, while a portfolio with a neutral correlation will have a
much lower probability of loss (of the order of 11%) while having an
improved risk/return ratio (of the order of 1.25, i.e. practically five
times the return of the former). Figure 13 (Appendix) represents the
risk-return of different asset classes. X-axis represent the period
analysed, while the y-axis represents the performance of each asset
selected in the analysis (Bridgewater Associates, LP, 2009).
...
The return on an asset i is equal to ri over a specified period. The
...
The weights in a Black-Litterman can be approached as (Clauss, 2011):
...
investor views in a relevant way. This approach adds to economic
predictions statistical uncertainty: the econometric approach is no more
a plug-in approach but a decision-bayesian approach (Clauss, 2011).
...
-   **Asset class: Equities (VTI):** VTI provides broad exposure to the
    global stock market, which is expected to benefit from economic
    growth.

-   **Asset class: Emerging market equities (EMGF):** EMGF provides
    exposure to emerging markets with favorable exposure to value,
    quality, momentum, and size factors. Emerging markets have the
    potential for higher growth than developed markets, but they also
    come with more risk. EMGF mitigates some of the risk by investing in
    companies with favorable exposure to specific factors.

-   **Asset class: Government bonds (IEF):** IEF provides exposure to
...
-   **Asset class: Commodities (DBC):** DBC provides exposure to a
    basket of commodities, which can be a good way to diversify a
    portfolio and hedge against inflation. However, it is important to
    note that commodities are volatile investments.

-   **Asset class: Precious metals (GLD):** GLD provides exposure to
    gold, which is a traditional safe haven asset that can provide
...
increased access for individual investors (Figure 1).
...
```{r Correlation-multi-asset-portfolio, fig.cap="Correlation analysis of the funds returns.", echo=FALSE, fig.pos='htbp'}
...
```{r GMV-plot, fig.cap="GMV portfolio weight. Each bar represents a fund weight from left to right: VTI, EMGF, IEF, DBC, GLD.", echo=FALSE, fig.pos='htbp'}
...
```{r PCA-three-factor-model, fig.cap="PCA portfolio weight. Each bar represent a fund weight (left to right): VTI, EMGF, IEF, DBC, GLD.", echo=FALSE, fig.pos='htbp'}
#PCA one factor
...
conviction and sentiment regarding each asset class (Figure 5).

```{r Plot-tangency-portoflio, fig.cap="GMV portfolio weight. Each bar represent a fund weight (left to right): VTI, EMGF, IEF, DBC, GLD.", echo=FALSE, fig.pos='htbp'}
...
the consensus view (Figure 6).

```{r Tatical-allocation-Omega-plot, fig.cap="BL portfolio weight. Each bar represent a fund weight (left to right): VTI, EMGF, IEF, DBC, GLD.", echo=FALSE, fig.pos='htbp'}
...
overall risk.](images/AWP_plot_4.png)
