...
run_simulation <- function(rf){
  ...
  # [snipped ... simulation remains the same]
  
  return(list(equity = equity, floor = floor, obpi_call = obpi_call, cppi = cppi))
}

# Merged plotting function
plot_scenario <- function(data){
  plot(data$equity, type = 'l', ylab = 'Strategies values', xlab = 'Months', ylim = c(60, 140))
  lines(data$floor, lty = 2)
  lines(data$obpi_call, col = 'blue')
  lines(data$cppi, col = 'green')
  leg.txt <- c("Equity", "Floor", "OBPI", "CPPI")
  legend(5, 138, leg.txt, col = c("black", "black", "blue", "green"), lty = c(1, 2, 1, 1), cex=0.8, bty="n")
}
...
base <- run_simulation(0.02)
increased_rf <- run_simulation(0.055)
decreased_rf <- run_simulation(0.005)

# Plotting each scenario
base_plot <- ggplotify::as.ggplot(expression(plot_scenario(base)))
increased_rf_plot <- ggplotify::as.ggplot(expression(plot_scenario(increased_rf)))
decreased_rf_plot <- ggplotify::as.ggplot(expression(plot_scenario(decreased_rf)))

# Arrange the plots side by side using gridExtra
grid.arrange(base_plot, increased_rf_plot, decreased_rf_plot, ncol = 3)
