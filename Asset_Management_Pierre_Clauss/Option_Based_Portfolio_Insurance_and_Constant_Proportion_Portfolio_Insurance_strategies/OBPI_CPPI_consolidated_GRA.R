...
volatility <- sd(r)
volatility_RNP <- sd(rPRN)
print(paste("Initial Volatility: ", round(volatility * 100, 2), "%"))
print(paste("Initial Volatility: ", round(volatility * 100, 2), "%"))
...
#Volatility for the stock under scenario one
volatility_1 <- sd(r)
print(paste("Scenario 1 Volatility: ", round(volatility_1 * 100, 2), "%"))
...
plot(equity, main = "Scenario 1: Risk-free rate goes up", type = 'l', ylab = 'Strategies values', xlab = 'Months', ylim = c(70, 130))
lines(floor, lty = 2)
lines(obpi_call, col = 'blue')
lines(cppi, col = 'green')
leg.txt <- c("Equity", "Floor", "OBPI", "CPPI")
...
#Volatility for the stock under scenario two
volatility_2 <- sd(r)
print(paste("Scenario 1 Volatility: ", round(volatility_1 * 100, 2), "%"))
...
{
  d0 <-
    ((rf_2 - sigma ^ 2 / 2) * (mat - (i - 1) * delta) + log(equityRNP[i] / strike)) /
    (sigma * sqrt(mat - (i - 1) * delta))
  d1 <- d0 + sigma * sqrt(mat - (i - 1) * delta)
  call[i] <-
    equityRNP[i] * pnorm(d1) - strike * exp(-rf_2 * (mat - (i - 1) * delta)) *
    pnorm(d0)
}
...
for (i in 1:(n + 1))
{
  floor[i] <- strike / (1 + rf_2) ^ (mat - (i - 1) * delta)
}
...
plot(equity, main = "Scenario 2: Risk-free rate goes down", type = 'l', ylab = 'Strategies values', xlab = 'Months', ylim = c(70, 130))
lines(floor, lty = 2)
lines(obpi_call, col = 'blue')
lines(cppi, col = 'green')
leg.txt <- c("Equity", "Floor", "OBPI", "CPPI")
...
##Scenario 3: Stress value goes up
...
plot(equity, main = "Scenario 3: Stress value goes up", type = 'l', ylab = 'Strategies values', xlab = 'Months', ylim = c(70, 130))
lines(floor, lty = 2)
lines(obpi_call, col = 'blue')
lines(cppi, col = 'green')
leg.txt <- c("Equity", "Floor", "OBPI", "CPPI")
...
##Scenario 4: Stress value go down
...
plot(equity, main = "Scenario 4: Stress value goes down", type = 'l', ylab = 'Strategies values', xlab = 'Months', ylim = c(70, 130))
lines(floor, lty = 2)
lines(obpi_call, col = 'blue')
lines(cppi, col = 'green')
leg.txt <- c("Equity", "Floor", "OBPI", "CPPI")
